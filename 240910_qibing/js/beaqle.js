...
function PrefRefTest(TestData) {
    ListeningTest.apply(this, arguments);
}
PrefRefTest.prototype = new ListeningTest();
PrefRefTest.prototype.constructor = PrefRefTest;


// implement specific code
PrefRefTest.prototype.createTestDOM = function (TestIdx) {
        ...
        if (!this.TestState.FileMappings[TestIdx]) {
        // Define possible choices and maximum choices
        const possibleValues = ["Z", "Y", "X", "W", "V"];
        // const maxChoices = 4;
        var maxChoices = this.TestConfig.Testsets[TestIdx].Length;

        // Slice to get the required number of choices
        const values = possibleValues.slice(0, maxChoices);
        ...
        // Create an object to hold the file mappings with dynamic keys
        this.TestState.FileMappings[TestIdx] = {};

        // Assign randomized values to dynamic keys
        values.forEach((value, index) => {
            // Use String.fromCharCode to get 'A', 'B', 'C', etc. based on ASCII values
            this.TestState.FileMappings[TestIdx][String.fromCharCode(65 + index)] = value;
            });
        }
        ...
        row  = tab.insertRow(-1);
        cell = row.insertCell(-1);
        cell.innerHTML = "<span class='testItem'>"+this.TestConfig.Testsets[TestIdx].Text+"</span>";
        cell.colSpan = Object.keys(this.TestState.FileMappings[TestIdx]).length+2; 
    
        // add reference audio if it is given
        row = tab.insertRow(-1);
        if (this.TestConfig.Testsets[TestIdx].Files["Reference"]) {
            fileID = "Reference"
            cell = row.insertCell(-1);
            cell.innerHTML =  '<button id="play'+fileID+'Btn" class="playButton" rel="'+fileID+'">Reference Audio</button>';
            cell.colSpan = Object.keys(this.TestState.FileMappings[TestIdx]).length; 
            this.addAudio(TestIdx, fileID, fileID);
        ...
        // Get the keys from the file mappings for the current test index
        var keys = Object.keys(this.TestState.FileMappings[TestIdx]);

        // Iterate through each key to create a cell and corresponding button
        keys.forEach((key, index) => {
            let fileID = this.TestState.FileMappings[TestIdx][key];
            let cell = row.insertCell(-1);
            // Set the inner HTML of the cell to include a button with dynamic IDs and labels
            cell.innerHTML = `<button id="play${fileID}Btn" class="playButton" rel="${fileID}">${key}</button>`;
            // Add the audio element or any other necessary setup
            this.addAudio(TestIdx, fileID, fileID);
        });
            
        cell = row.insertCell(-1);
        cell.innerHTML = "<button class='stopButton'>Stop</button>";
        
        cell = row.insertCell(-1);
        cell.innerHTML = "Press buttons to start/stop playback."; 
...
//         cell[4].innerHTML = "Please select the item with the 'LOWEST' intensity.";
        ...
        var numberOfChoices = Object.keys(this.TestState.FileMappings[TestIdx]).length;
        // Insert cells and radio buttons dynamically
        for (let i = 0; i < numberOfChoices; i++) {
            let cell = row.insertCell(-1);
            let label = String.fromCharCode(65 + i); // Converts 0 to 'A', 1 to 'B', etc.
            cell.innerHTML = `<input type='radio' name='Lselection' id='Lselect${label}'/>`;
        }
        cell = row.insertCell(-1);
        cell = row.insertCell(-1);
        cell.innerHTML = "Please select the item with"+this.TestConfig.Testsets[TestIdx].LowText+".";
        ...
        for (let i = 0; i < numberOfChoices; i++) {
            let cell = row.insertCell(-1);
            let label = String.fromCharCode(65 + i); // Converts 0 to 'A', 1 to 'B', etc.
            cell.innerHTML = `<input type='radio' name='Hselection' id='Hselect${label}'/>`;
        }
        cell = row.insertCell(-1);
        cell = row.insertCell(-1);
        cell.innerHTML = "Please select the item with"+this.TestConfig.Testsets[TestIdx].HighText+".";
...
PrefRefTest.prototype.readRatings = function (TestIdx) {
    var ratings = this.TestState.Ratings[TestIdx];
    var selectL = ratings.charAt(0);
    var selectH = ratings.charAt(1);
...
PrefRefTest.prototype.saveRatings = function (TestIdx) {
    var labels = ['A', 'B', 'C', 'D', 'E']; // Extend labels as needed for up to 5 choices
    var selectL = '';
    var selectH = '';

    // Loop through each label to determine which 'L' selection is checked
    for (var i = 0; i < labels.length; i++) {
        if ($("#Lselect" + labels[i]).prop("checked")) {
            selectL = labels[i];
            break; // Exit the loop once the selected label is found
        }
    }

    // Loop through each label to determine which 'H' selection is checked
    for (var i = 0; i < labels.length; i++) {
        if ($("#Hselect" + labels[i]).prop("checked")) {
            selectH = labels[i];
            break; // Exit the loop once the selected label is found
        }
    }

    // Save the combined ratings into the test state
    this.TestState.Ratings[TestIdx] = selectL + selectH;
}

// PrefRefTest.prototype.formatResults = function () {
...
PrefRefTest.prototype.formatResults = function () {
    ...
    var numCorrect = 0;  // Not used in the provided code, potentially for future implementation
    var numWrong = 0;    // Not used in the provided code, potentially for future implementation

    // Evaluate single tests
    for (var i = 0; i < this.TestConfig.Testsets.length; i++) {
        this.TestState.EvalResults[i] = {};
        this.TestState.EvalResults[i].TestID = this.TestConfig.Testsets[i].TestID;
        ...
        if (this.TestState.TestSequence.indexOf(i) >= 0) {
            ...
            // Dynamically construct presentation order string
            let presentationOrder = Object.keys(this.TestState.FileMappings[i]).reduce((acc, key) => {
                return `${acc}${key}=${this.TestState.FileMappings[i][key]}, `;
            }, '').slice(0, -2);  // Removes the trailing comma and space

            cell = row.insertCell(-1);
            this.TestState.EvalResults[i].PresentationOrder = presentationOrder;
            cell.innerHTML = presentationOrder;

            cell = row.insertCell(-1);
            this.TestState.EvalResults[i].Runtime = this.TestState.Runtime[i];
            cell.innerHTML = this.TestState.Runtime[i];
